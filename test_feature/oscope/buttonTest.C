...
//______________________________________________________________________________
int planeid(int ch){
  // SBND: 1984 U + 1984 V + 1664 W = 5632 per TPC
  int ch1 = ch % 5632;
  if (ch1 < 1984) return 0;
  else if (ch1 < 3968) return 1;
  else return 2;
}
...
void ButtonWindow::init()
{
   ...
   int apaid = ch / 5632;  // TPC index
   const char* orig_tag = Form("h%s_orig%d",label,apaid);
   const char* raw_tag = Form("h%s_raw%d",label,apaid);
   const char* wiener_tag = Form("h%s_wiener%d",label,apaid);
   const char* gauss_tag = Form("h%s_gauss%d",label,apaid);
   const char* troi_tag = Form("h%s_tight_lf%d",label,apaid); // tight roi
   const char* lroi_tag = Form("h%s_loose_lf%d",label,apaid); // loose roi
   const char* croi_tag = Form("h%s_cleanup_roi%d",label,apaid);
   const char* broi1_tag = Form("h%s_break_roi_1st%d",label,apaid);
   const char* broi2_tag = Form("h%s_break_roi_2nd%d",label,apaid);
   const char* sroi_tag = Form("h%s_shrink_roi%d",label,apaid);
   const char* eroi_tag = Form("h%s_extend_roi%d",label,apaid);
